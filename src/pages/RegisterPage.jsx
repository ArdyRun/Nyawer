...
export default function RegisterPage() {
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center px-4">
        <div className="glass-card p-8 rounded-xl max-w-md w-full text-center">
          <div className="w-12 h-12 rounded-lg flex items-center justify-center bg-zinc-900 border border-zinc-800 mx-auto mb-4">
            <CheckCircle2 size={24} className="text-violet-400" />
          </div>
          <h2 className="font-display font-bold text-xl text-zinc-100 mb-2">Cek Email Anda</h2>
          <p className="text-zinc-400 text-xs mb-6 leading-relaxed">
            Link konfirmasi sudah dikirim ke{' '}
            <strong className="text-zinc-200">{form.email}</strong>.
            Klik link tersebut untuk mengaktifkan akun.
          </p>
          <Link to="/login" className="btn-neon px-6 py-2.5 rounded-lg text-xs font-semibold inline-flex items-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center px-4 relative overflow-hidden py-10">
      {/* Background grid */}
      ...
      <div className="relative z-10 w-full max-w-sm">
        <div className="glass-card p-6 md:p-8">
          ...
          <h1 className="font-display font-bold text-lg text-zinc-100 mb-1 text-center">Buat akun baru</h1>
          <p className="text-zinc-500 text-xs mb-6 text-center">Gratis selamanya. Potongan flat 4% saja.</p>
                ...
                <div className="flex gap-1.5 mt-2">
                  {[6, 8, 12].map((len, i) => (
                    <div key={i} className="flex-1 h-1 rounded-full transition-all"
                      style={{ background: form.password.length >= len ? (i === 2 ? '#10b981' : i === 1 ? '#f59e0b' : '#ef4444') : 'rgba(255,255,255,0.06)' }} />
                  ))}
                </div>
              )}
            </div>
        ...
        {!isSupabaseReady && (
          <div className="mt-4 text-center">
            <p className="text-[10px] text-zinc-600">
              Mode Demo - isi form dengan data apa saja untuk mencoba
            </p>
          </div>
        )}
      </div>
